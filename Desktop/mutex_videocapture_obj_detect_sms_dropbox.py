...
def VideoStreamRead(queue, event,  src="rtsp://admin:Misincu2b@86.228.137.47:554/12"):
    lastUploaded = datetime.datetime.now()
...
def VideoStreamDetect(queue, event ):               
...
#Lancement du process loop
    print('loop to be started')
    pipeline = queue.Queue(maxsize=10)
    event = threading.Event()
    with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
        executor.submit(VideoStreamRead, pipeline, event)
        executor.submit(VideoStreamDetect, pipeline, event)
        time.sleep(0.1)
        event.set()
